...
mod tests {
    ...
    fn test_parse_workflow_yaml() {
        ...
        let workflow_path = PathBuf::from("./documents/workflow.yaml");
        ...
        if !workflow_path.exists() {
            ...
            assert_eq!(workflow.entry_graph_id, Some("a2df1dee-eba2-11f0-b126-7c70db10a7e3".to_string()));

            // Check that the workflow graph has nodes
            assert!(workflow.graph.node_count() > 0, "Workflow should have at least one node");
            ...
            for node_idx in workflow.graph.node_indices() {
                ...
                assert!(!node.subgraph.is_empty(), "Node subgraph should not be empty");
            }

            println!("Successfully parsed workflow with {} nodes", workflow.graph.node_count());
        } else {
    ...
    #[test]
    fn test_parse_workflow_yaml_structure() {
        // Test that the workflow contains expected nodes from the YAML
        let current_dir = std::env::current_dir().expect("Failed to get current directory");
        let workflow_path = current_dir.join("documents").join("workflow.yaml");
        ...
        assert!(csv_reader_found, "CsvReader node should be present in the workflow");
        assert!(rename_attributes_found, "RenameAttributes node should be present in the workflow");
        assert!(prepare_extra_attr_found, "PrepareExtraAttribute node should be present in the workflow");

        println!("Found expected nodes: CsvReader={}, RenameAttributes={}, PrepareExtraAttribute={}",
